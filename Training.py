...
PATH = '/Users/harsha/Desktop/PhD_project/Updated_Model/GAN_Weights.pt' #'./HDir_Gan200_1'
#Keyword = "NormPara" #or "NormPara X1"
...
Para_dim = 64              #(condition+input vector)
cond_dim=49
InputVector_dim=15
netD = Discriminator(Para_dim).to(device)
netG = Generator(Para_dim).to(device)
...
fileName = '/Users/harsha/Desktop/PhD_project/Updated_Model/Inputdata_Normalised'
...
for epoch in range(num_epochs):
  ...
  for i in range(Niter):
    ...
    noise = torch.randn(b_size, InputVector_dim, device=device)
    g_input = torch.cat((noise, real_c), dim=1)
    fake = netG(g_input)
